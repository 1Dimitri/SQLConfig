...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Launch maintenance solution', 
                ...
                @command=N'sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d <%= $PLASTER_PARAM_DBMaintenance %> -Q "EXECUTE [dbo].[IndexOptimize] @Databases = ''ALL_DATABASES'',@FragmentationLow = NULL,@FragmentationMedium = NULL,@FragmentationHigh = NULL,@UpdateStatistics = ''ALL'',@LogToTable=''Y''" -b', 
                @output_file_name=N'<%= $PLASTER_PARAM_SQLLogPath %>\UpdateStatistics_$(ESCAPE_SQUOTE(JOBID))_$(ESCAPE_SQUOTE(STEPID))_$(ESCAPE_SQUOTE(STRTDT))_$(ESCAPE_SQUOTE(STRTTM)).txt', 
                @flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Notify operator OK', 
                ...
                @command=N'EXECUTE msdb.dbo.sp_notify_operator @name=N''<%= $PLASTER_PARAM_Operator %>'',@subject=N''[<%= $PLASTER_PARAM_FriendlyName %>] [<%= $PLASTER_PARAM_InstanceName %>] [OK] UpdateStats - ALL_DATABASES'',@body=N''All Databases from <%= $PLASTER_PARAM_InstanceName %> have their statistics updated.''
', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Notify Operator Error', 
                ...
                @retry_interval=0, 
                @os_run_priority=0, @subsystem=N'TSQL', 
                @command=N'EXECUTE msdb.dbo.sp_notify_operator @name=N''<%= $PLASTER_PARAM_Operator %>'',@subject=N''[<%= $PLASTER_PARAM_FriendlyName %>] [<%= $PLASTER_PARAM_InstanceName %>] [KO] UpdateStats - ALL_DATABASES'',@body=N''Some databases in <%= $PLASTER_PARAM_InstanceName %> failed to have their statistics updated. Please check the CommandLog table in <%= $PLASTER_PARAM_DBMaintenance %> to know more about the error''
', 
                @database_name=N'<%= $PLASTER_PARAM_DBMaintenance %>', 
                @flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
