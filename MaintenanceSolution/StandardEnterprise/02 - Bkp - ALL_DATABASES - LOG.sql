...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Bkp - ALL_DATABASES - LOG - <%= $PLASTER_PARAM_SQLBackupPath %>', 
                @enabled=1, 
                ...
                @notify_level_page=0, 
                @delete_level=0, 
                @description=N'Do a log backup of all database using Ola''s solution', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Launch backup solution', 
                ...
                @command=N'sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d <%= $PLASTER_PARAM_DBMaintenance %> -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''ALL_DATABASES'', @Directory = N''<%= $PLASTER_PARAM_SQLBackupPath %>'', @BackupType = ''LOG'', @Verify = ''Y'', @CleanupTime = 48, @CheckSum = ''Y'', @LogToTable = ''Y''" -b', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Notify operator OK', 
                ...
                @command=N'EXECUTE msdb.dbo.sp_notify_operator @name=N''<%= $PLASTER_PARAM_operator %>'',@subject=N''[<%= $PLASTER_PARAM_FriendlyName %>] [<%= $PLASTER_PARAM_InstanceName %>] [OK] ALL DATABASES - LOG - <%= $PLASTER_PARAM_SQLBackupPath %>'',@body=N''All Databases backups from <%= $PLASTER_PARAM_InstanceName %> on <%= $PLASTER_PARAM_SQLBackupPath %> have been done successfully. Congratulations !''
', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Notify Operator Error', 
                ...
                @retry_interval=0, 
                @os_run_priority=0, @subsystem=N'TSQL', 
                @command=N'EXECUTE msdb.dbo.sp_notify_operator @name=N''<%= $PLASTER_PARAM_operator %>'',@subject=N''[<%= $PLASTER_PARAM_FriendlyName %>] [<%= $PLASTER_PARAM_InstanceName %>] [KO] ALL DATABASES - LOG - <%= $PLASTER_PARAM_SQLBackupPath %>'',@body=N''All Databases backups from <%= $PLASTER_PARAM_InstanceName %> on <%= $PLASTER_PARAM_SQLBackupPath %> failed. Please check the CommandLog table in <%= $PLASTER_PARAM_DBMaintenance %> to know more about the error''
', 
                @database_name=N'<%= $PLASTER_PARAM_DBMaintenance %>', 
                @flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Schedule for Bkp - ALL_DATABASES - LOG - <%= $PLASTER_PARAM_SQLBackupPath %>', 
-- Mo,Fr, 7am to 6:59pm every 4 hrs,
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=62, 
		@freq_subday_type=8, 
...
-- schedule id to be generated by the server
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
